...
var ExampleApplication = React.createClass({
  ...
  render: function () {
    ...
    var mynode = <g>
      {
        data.node.map(function (v, i) {
          return <path key={i} id='test' d={v.value} stroke="black" strokeWidth="1" fill="transparent" />
        })
      }
    </g>

    var mylink = <path id='test' d={data.link} stroke="black" strokeWidth="1" fill="transparent" />;
    return <svg onMouseUp={that.onmouseup} onMouseMove={that.onmousemove} onMouseDown={that.onmousedown}
      id="mysvg" style={{ "position": "absolute", "top": "0", "left": "0" }} width="2000" height="2000" viewBox="0 0 2000 2000" xmlns="http://www.w3.org/2000/svg">
      {mynode}{mylink}
    </svg>
  }
});
...
ReactDOM.render(
  ...
  document.getElementById('mysvgdiv')
);
