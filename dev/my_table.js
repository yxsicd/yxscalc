...
var ExampleApplication = React.createClass({
    ...
    render: function () {
        ...
        {
            tharr.push(React.createElement("th", { "key": "th_" + i }, that.state.data.keys[i]));
        }
        trarr.push(React.createElement("tr", { "key": "head" }, tharr));
        ...
        for (var i = 0; i < rows_show.length; i++)
        {
            ...
            {
                trtharr.push(React.createElement("td", { "key": "td_" + i + "_" + j }, row[j]))
            }
            trarr.push(React.createElement("tr", { "key": "tr_" + i }, trtharr));
            ...
            for (var j = 0; j < row.length; j++)
            {
                var etd=[];
                etd.push(
                    re("td", { "key": j }, re("lable", {key:"l_rc", htmlFor: "rescount" }, "count")),
                    re("td", { "key": j+"1" }, row[j])
                )
                etr.push(re("tr",null,etd));
            }
            var edit_table = re("table",null,(re("tbody",null,etr)));
            trarr.push(React.createElement("tr", { "key": "tr_e" + i }, edit_table));
